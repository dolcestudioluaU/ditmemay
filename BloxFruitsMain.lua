...
if _Effect then
    ...
    if _Death then
        local deathRequireSuccess, deathRequireFunc = pcall(require, _Death)

        if deathRequireSuccess and type(deathRequireFunc) == 'function' then
            hookfunction(deathRequireFunc, function() end)
        end
    end
    ...
    if _Respawn then
        local respawnRequireSuccess, respawnRequireFunc = pcall(require, _Respawn)

        if respawnRequireSuccess and type(respawnRequireFunc) == 'function' then
            hookfunction(respawnRequireFunc, function() end)
        end
    end
end

local _Util = game:GetService('ReplicatedStorage'):WaitForChild('Util', 5)
local cameraShaker = _Util and _Util:FindFirstChild('CameraShaker')
...
CheckQuest = function()
    local quest = GetQuests()
    if quest and quest.Mob then
        Mon = quest.Mob
        NameMon = quest.Mob
        ...
        if GuideModule and GuideModule.Data and GuideModule.Data.NPCList then
            for npcName, npcData in pairs(GuideModule.Data.NPCList) do
                if npcData.Levels then
                    for _, level in pairs(npcData.Levels) do
                        if level == quest.LevelReq then
                            CFrameQuest = npcData.CFrame
                            break
                        end
                    end
                end
                if CFrameQuest then break end
            end
        end
        if not CFrameQuest then
            local npcCFrame = GetQuestPoint()
            if npcCFrame then
                CFrameQuest = npcCFrame
            end
        end

        if game.Workspace:FindFirstChild("_WorldOrigin") then
            local enemySpawns = game.Workspace._WorldOrigin:FindFirstChild("EnemySpawns")
            if enemySpawns then
                for _, spawnPart in pairs(enemySpawns:GetChildren()) do
                    if spawnPart:IsA("Part") and string.find(spawnPart.Name, Mon) then
                        CFrameMon = spawnPart.CFrame
                        break
                    end
                end
            end
        ...
        if not CFrameMon then
            if game.Workspace:FindFirstChild("Enemies") then
                for _, enemy in pairs(game.Workspace.Enemies:GetChildren()) do
                    if enemy.Name == Mon and enemy:FindFirstChild("HumanoidRootPart") then
                        CFrameMon = enemy.HumanoidRootPart.CFrame
                        break
                    end
                end
            end
        ...
        if not CFrameQuest and CFrameMon then
            CFrameQuest = CFrameMon
        end
    end
end

local ItemRepService, ItemConfig, CombatUtil
pcall(function() ItemRepService = require(game:GetService('ReplicatedStorage'):WaitForChild('ItemReplicationService')) end)
pcall(function() ItemConfig = require(game:GetService('ReplicatedStorage'):WaitForChild('ItemConfig')) end)
pcall(function() CombatUtil = require(game:GetService('ReplicatedStorage').Modules:WaitForChild('CombatUtil')) end)
...
function GetInventoryItems()
    ...
    if not ItemRepService then
        return list
    end

    local ok, cache = pcall(function()
        return ItemRepService.UserCache[game:GetService('Players').LocalPlayer.UserId]
    end)
    ...
    local okQty, quantities = pcall(function()
        return cache:GetItems('Quantity')
    end)

    if not okQty or not quantities then
        return list
    end
    ...
    pcall(function()
        for _, v in pairs(cache:GetItems('Mastery') or {}) do
            mastery[v.ItemId] = v.Value
        end
    end)

    for _, v in pairs(quantities) do
        local id, qty = v.ItemId, v.Value
        local itemType, label = '?', ''

        if ItemConfig then
            pcall(function()
                local c = ItemConfig.match(id):unwrap()

                if c and c.Index then
                    itemType = c.Index.IdType
                    label = c.Index.DebugLabel or ''
                end
            end)
        end

        local name = CleanItemName(label)
        local weaponType

        if itemType == 'Moveset' and CombatUtil then
            pcall(function()
                local data = CombatUtil:GetWeaponData(name)
...
function GetCountItem(itemName)
    for _, v in ipairs(GetInventoryItems()) do
...
function GetWeaponInventory(weaponName)
    for _, v in ipairs(GetInventoryItems()) do
        if v.Name == weaponName and (v.Type == 'Moveset' or v.Type == 'PhysicalMoveset') then
            return true
        end
    end
    return false
...
function PrintInventory()
    ...
    local swords, guns, melees, fruits, accessories, materials = {}, {}, {}, {}, {}, {}

    for _, v in ipairs(items) do
        local entry = v.Name .. ' (x' .. tostring(v.Count) .. ')' .. (v.Mastery and (' [Mastery ' .. tostring(v.Mastery) .. ']') or '')

        if v.Type == 'Material' then
            table.insert(materials, entry)
        elseif v.Type == 'Accessory' then
            table.insert(accessories, entry)
        elseif v.Type == 'PhysicalMoveset' then
            table.insert(fruits, entry)
        elseif v.Type == 'Moveset' then
            if v.WeaponType and string.find(v.WeaponType, 'Sword') then
                table.insert(swords, entry)
            elseif v.WeaponType and string.find(v.WeaponType, 'Gun') then
                table.insert(guns, entry)
            else
                table.insert(melees, entry)
            end
        end
    end
    ...
    local function dump(t, title)
        ...
        print(string.format('=== %s (%d) ===', title, #t))
        table.sort(t)
    ...
    print('===== ' .. LP.Name .. ' =====')
    print(string.format('Lv %s | Beli %s | Frag %s', tostring(LP.Data.Level.Value), tostring(LP.Data.Beli.Value), tostring(LP.Data.Fragments and LP.Data.Fragments.Value or 0)))
    print('Fruit: ' .. (LP.Data.DevilFruit.Value ~= '' and LP.Data.DevilFruit.Value or '(none)') .. '\n')
    dump(swords, 'SWORDS')
    dump(guns, 'GUNS')
    dump(melees, 'MELEE')
    dump(fruits, 'FRUITS')
    dump(accessories, 'ACCESSORIES')
    dump(materials, 'MATERIALS')
end
...
function MaterialMon()
    ...
    if _Character then
        local function requestEntranceIfFar(targetPosition, thresholdDistance)
            if thresholdDistance <= (_Character.Position - targetPosition).Magnitude then
        ...
        if World1 then
            if SelectMaterial ~= 'Angel Wings' then
                if SelectMaterial ~= 'Leather + Scrap Metal' then
                    if SelectMaterial ~= 'Magma Ore' then
                        if SelectMaterial == 'Fish Tail' then
                            ...
                            requestEntranceIfFar(Vector3.new(61163.8515625, 5.342342376708984, 1819.7841796875), 17000)
                        end
                    else
                        MMon = {
                            'Military Soldier',
                ...
                requestEntranceIfFar(Vector3.new(-4607.82275, 872.54248, -1667.55688), 10000)
            end
        elseif World2 then
            if SelectMaterial ~= 'Leather + Scrap Metal' then
                if SelectMaterial ~= 'Magma Ore' then
                    if SelectMaterial ~= 'Ectoplasm' then
                        ...
                        requestEntranceIfFar(Vector3.new(61163.8515625, 5.342342376708984, 1819.7841796875), 18000)
                    end
                else
                    MMon = {
                        'Magma Ninja',
...
function AutoHaki()
    if not game:GetService('Players').LocalPlayer.Character:FindFirstChild('HasBuso') then
        ...
        if hakiCooldown <= currentTick - lastHakiTime then
            game:GetService('ReplicatedStorage').Remotes.CommF_:InvokeServer('Buso')

            lastHakiTime = currentTick
        end
    end
end
...
function UnEquipWeapon(toolName)
    ...
    if unequipCooldown <= currentTick - lastUnequipTime then
        if game.Players.LocalPlayer.Character:FindFirstChild(toolName) then
            getgenv().NotAutoEquip = true
            game.Players.LocalPlayer.Character[toolName].Parent = game.Players.LocalPlayer.Backpack
            getgenv().NotAutoEquip = false
        end

        lastUnequipTime = currentTick
    end
end
...
function EquipWeapon(toolName)
    ...
    if equipCooldown <= currentTick - lastEquipTime then
        local toolToEquip = not getgenv().NotAutoEquip and game.Players.LocalPlayer.Backpack:FindFirstChild(toolName)

        if toolToEquip then
            game.Players.LocalPlayer.Character.Humanoid:EquipTool(toolToEquip)
        end

        lastEquipTime = currentTick
    end
end
function BTP(destinationCFrame)
    ...
    local _ = destinationCFrame.Position
    local _Position = _HumanoidRootPart.Position

    while true do
        _Humanoid.Health = 0
        _HumanoidRootPart.CFrame = destinationCFrame
        ...
        if (destinationCFrame.Position - _HumanoidRootPart.Position).Magnitude <= 2000 then
            return
        end
    end
end
function BTPZ(destinationCFrame)
    ...
    if _LocalPlayer4.Character and _LocalPlayer4.Character:FindFirstChild('HumanoidRootPart') then
        _LocalPlayer4.Character.HumanoidRootPart.CFrame = destinationCFrame
    end
end

local lastFastPosTime = 0
local fastPosCooldown = 0.5

function fastpos(destinationCFrame)
    local v46 = tick()

    if fastPosCooldown <= v46 - lastFastPosTime then
        local _Magnitude = (destinationCFrame.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude

        game:GetService('TweenService'):Create(game:GetService('Players').LocalPlayer.Character.HumanoidRootPart, TweenInfo.new(_Magnitude / 1000, Enum.EasingStyle.Linear), {CFrame = destinationCFrame}):Play()
...
function TPB(destinationCFrame, boat)
    local currentTick = tick()

    if tpBCooldown <= currentTick - lastTPBTime then
        local _TweenService = game:GetService('TweenService')
        local _Magnitude2 = (boat.CFrame.Position - destinationCFrame.Position).Magnitude
        local speedBoat = getgenv().SpeedBoat
        local v56 = TweenInfo.new(_Magnitude2 / speedBoat, Enum.EasingStyle.Linear)
        ...
        local tweenInstance = _TweenService:Create(boat, v56, {CFrame = destinationCFrame})
        local tweenAlias = tweenInstance

        tweenInstance.Play(tweenAlias)

        lastTPBTime = currentTick
...
function EquipAllWeapon()
    pcall(function()
        ...
        if equipAllCooldown <= v61 - lastEquipAllTime then
            ...
            lastEquipAllTime = v61
        end
    end)
end
function CheckNearestTeleporter(startPosition)
    local _ = game.Players.LocalPlayer.Data.Level.Value
    local _Position2 = startPosition.Position
    local _huge = math.huge
    local _huge2 = math.huge
    local _PlaceId2 = game.PlaceId
    local isCurrentWorld1 = nil
    local isCurrentWorld2 = nil
    local isCurrentWorld3 = nil

    if _PlaceId2 == 2753915549 then
        isCurrentWorld1 = true
    elseif _PlaceId2 == 4442272183 then
        isCurrentWorld2 = true
    elseif _PlaceId2 == 7449423635 then
        isCurrentWorld3 = true
    end

    local v75 = {}
    local teleporterPositions

    if isCurrentWorld3 then
        teleporterPositions = {
    ...
    elseif isCurrentWorld2 then
        ...
        teleporterPositions = isCurrentWorld1 and {
    ...
    local v77, v78, v79 = pairs(teleporterPositions)
    local distanceMap = {}
    ...
    while true do
        ...
        distanceMap[v79] = (v81 - _Position2).Magnitude
    end

    local v82, v83, v84 = pairs(distanceMap)
    ...
    local v87, v88, v89 = pairs(distanceMap)
    local nearestTeleporterCFrame = nil
    ...
    while true do
        ...
        if v91 <= _huge then
            nearestTeleporterCFrame = teleporterPositions[v89]
        end
    end

    if _huge2 <= (_Position2 - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude then
        return nearestTeleporterCFrame
    end
end
function requestEntrance(entrancePoint)
...
function topos(targetPosition)
    pcall(function()
        if game:GetService('Players').LocalPlayer and (game:GetService('Players').LocalPlayer.Character and (game:GetService('Players').LocalPlayer.Character:FindFirstChild('Humanoid') and (game:GetService('Players').LocalPlayer.Character:FindFirstChild('HumanoidRootPart') and (game:GetService('Players').LocalPlayer.Character.Humanoid.Health > 0 and game:GetService('Players').LocalPlayer.Character.HumanoidRootPart)))) then
            ...
            DefualtY = targetPosition.Y
            TargetY = targetPosition.Y
            targetCFrameWithDefualtY = CFrame.new(targetPosition.X, DefualtY, targetPosition.Z)
            ...
            local nearestTeleporter = CheckNearestTeleporter(targetPosition)
            ...
            if IngoreY and (b1.Position - targetCFrameWithDefualtY.Position).Magnitude > 5 then
                ...
                local v100 = {CFrame = targetPosition}
...
local function isAlive(characterModel)
    ...
    local _Humanoid2 = characterModel:FindFirstChild('Humanoid')
...
local function getNearbyTargets(originObject, radius)
    local enemyChildren = game:GetService('Workspace').Enemies:GetChildren()
    local playerList = game:GetService('Players'):GetPlayers()
    local _Position3 = originObject:GetPivot().Position
    local v122, v123, v124 = ipairs(enemyChildren)
    local nearbyTargets = {}

    while true do
        local enemyChild

        v124, enemyChild = v122(v123, v124)

        if v124 == nil then
            break
        end

        local _HumanoidRootPart2 = enemyChild:FindFirstChild('HumanoidRootPart')

        if _HumanoidRootPart2 and (isAlive(enemyChild) and (_HumanoidRootPart2.Position - _Position3).Magnitude <= radius) then
            table.insert(nearbyTargets, enemyChild)
        end
    end

    local v128, v129, v130 = ipairs(playerList)

    while true do
        local player

        v130, player = v128(v129, v130)

        if v130 == nil then
            break
        end
        if player ~= _LocalPlayer6 and player.Character then
            local _HumanoidRootPart3 = player.Character:FindFirstChild('HumanoidRootPart')

            if _HumanoidRootPart3 and (isAlive(player.Character) and (_HumanoidRootPart3.Position - _Position3).Magnitude <= radius) then
...
function AttackNoCoolDown()
    ...
    while true do
        ...
        v137, currentTool = v135(v136, v137)
    ...
    if currentTool then
        local nearbyTargets = getNearbyTargets(_Character2, 60)

        if currentTool:FindFirstChild('LeftClickRemote') then
            local targetIter, targetIterState, targetIterIndex = ipairs(nearbyTargets)
            local targetIndex = 1

            while true do
                local targetIterKey, targetCharacter = targetIter(targetIterState, targetIterIndex)

                if targetIterKey == nil then
                    break
                end

                targetIterIndex = targetIterKey

                local _HumanoidRootPart4 = targetCharacter:FindFirstChild('HumanoidRootPart')

                if _HumanoidRootPart4 then
                    local _Unit = (_HumanoidRootPart4.Position - _Character2:GetPivot().Position).Unit

                    currentTool.LeftClickRemote:FireServer(_Unit, targetIndex)

                    targetIndex = targetIndex + 1
                end
            end
        else
            local targetIter2, targetIterState2, targetIterIndex2 = ipairs(nearbyTargets)
            local targetHeadPairs = {}
            local lastHead = nil

            while true do
                local v154, v155 = targetIter2(targetIterState2, targetIterIndex2)

                if v154 == nil then
                    break
                end

                targetIterIndex2 = v154

                if not v155:GetAttribute('IsBoat') then
                    local _Head = v155:FindFirstChild('Head')

                    if _Head then
                        table.insert(targetHeadPairs, {v155, _Head})
            ...
            if lastHead then
                ...
                pcall(function()
                    ...
                    _RERegisterHit:FireServer(lastHead, targetHeadPairs)
                end)
            end
        end
    end
...
local savedElapsed = nil
local savedWatermark = nil
local loadingLabel = nil
local isLoaded = false
local searchText = nil
local loadingProcess = false
local currentStep = 0
local totalSteps = 50
    ...
    task.spawn(function()
        while true do
            local success,err = pcall(function()
                ...
                if DevConsole then
                    for index,label in pairs(DevConsole:GetDescendants()) do 
...
local function setSuccessText(label)
    local success, err = pcall(function()
        if label and label:IsA("TextLabel") and savedColor and savedWatermark and savedElapsed then
            local currentTimestamp = ""
            if label.Text:match("^(%d+:%d+:%d+)") then
                currentTimestamp = label.Text:match("^(%d+:%d+:%d+)") .. " -- "
            end
            label.Text = string.format("%s<font color='rgb(%s)' size='15'>[%s] Successfully loaded in %ds</font>", currentTimestamp, Modules.Colors[savedColor], savedWatermark, savedElapsed)
        end
    end)
...
Modules.LoadingBar = function(watermark, color, loadingsymbol)
    savedColor = color
    ...
    searchText = watermark..tostring(math.random(500,20000))
    ...
    task.spawn(function()
        ...
        local delay = totalDuration / totalSteps

        for i = 1, totalSteps do
            currentStep = i
            if loadingLabel then
                local filled = math.floor(currentStep * 20 / totalSteps)
                local bar = string.rep("█", filled) .. string.rep("░", 20 - filled)
                local percent = math.floor(currentStep * 100 / totalSteps)
                local currentTimestamp = ""
                if loadingLabel.Text:match("^(%d+:%d+:%d+)") then
                    currentTimestamp = loadingLabel.Text:match("^(%d+:%d+:%d+)") .. " -- "
                end
                loadingLabel.Text = string.format("%s<font color='rgb(%s)' size='15'>Loading [%s] %d%%</font>", currentTimestamp, Modules.Colors["White"], bar, percent)
            end
            task.wait(delay)
        end

        savedElapsed = os.time() - start
        isLoaded = true
        loadingProcess = false

        if loadingLabel then
            setSuccessText(loadingLabel)
        end
    end)
    ...
    task.spawn(function()
        while true do
            task.wait(0.1)
            local DevConsole = Modules.Services.CoreGui:FindFirstChild("DevConsoleMaster")
            if DevConsole and searchText then
                for index,label in pairs(DevConsole:GetDescendants()) do 
                    if label:IsA("TextLabel") then 
                        label.RichText = true 
                        if string.find(label.Text:lower(), searchText:lower()) then
                            loadingLabel = label
                            if isLoaded then
...
ImageButton.Image = "http://www.roblox.com/asset/?id=108024169715822"
...
Fluent:Notify({
        Title = "Notification",
        Content = "Dolce Hub - Blox Fruit",
        SubContent = "Anh Em Chơi Vui Vẻ Nhé !", -- Optional
        ...
        if getgenv().AutoFarm and (FarmMode == 'Farm Level' and v370) then
            spawn(function()
                ...
                if not _Visible then
                    ...
                    if CFrameQuest and _Character4 then
                        if BypassTP then
                ...
                if _Visible then
                    ...
                    if not mobFound and CFrameMon then
                        topos(CFrameMon)
                    end
                end
            end)
...
Modules.LoadingBar("Dolce Hub","Green","#")
